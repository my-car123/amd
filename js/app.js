// ======================== الجزء 1 من 4 ========================
// app.js - الإصدار النهائي الكامل (جميع الوظائف)
...
// --- i18n (كاملة مع جميع المفاتيح الجديدة) ---
const translations = {
...
// --- ربط العهدة مع التحذير وتخزين الاسم ---
...
async function initDriverDashboardRealtime() {
  ...
  try {
    ...
    driverUnsubscribe = onValue(custodyRef, async () => { await refreshDriverCars(); });
  ...
  if(user) {
    try {
      if(user.email === 'saad323m@gmail.com') {
        ...
        document.getElementById('nav-mods').style.display = 'flex';
