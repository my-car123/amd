...
async function refreshDashboardStats() {
    try {
        ...
        for (let id in elements) {
            ...
            if (el) el.textContent = String(elements[id]).replace(/[٠-٩]/g, d => '٠١٢٣٤٥٦٧٨٩'.indexOf(d));
        }
...
function renderVehicleAccordionCard(targetContainer, v, statusReg, statusIns) {
    ...
    targetContainer.innerHTML = targetContainer.innerHTML.replace(/[٠-٩]/g, d => '٠١٢٣٤٥٦٧٨٩'.indexOf(d));
}
...
async function processNewVehicleSaving(e, uid, seqNum) {
    ...
    try {
        // فحص ومنع تكرار اللوحة أو رقم VIN برمجياً على مستوى السيرفر اللحظي
        const snapshot = await rGet(rChild(rRef(window.firebaseDB), "vehicles"));
        if(snapshot.exists()) {
            let isDup = false;
...
window.addEventListener('DOMContentLoaded', () => { refreshDashboardStats(); onDashboardTabChanged(localStorage.getItem('active_dash_tab') || 'overview'); });
